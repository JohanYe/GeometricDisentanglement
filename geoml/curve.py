...
class DiscreteCurve(BasicCurve):
    def __init__(self, begin, end, num_nodes=5, device=None, requires_grad=True):
        ...
        self.t = torch.linspace(0, 1, num_nodes, dtype=begin.dtype)[1:-1].reshape((-1, 1)) # (num_nodes-2)x1
        self.num_nodes = num_nodes
        self.begin = begin.reshape((1, -1))
        self.end = end.reshape((1, -1))
        self.parameters = torch.tensor(self.t.mm(self.end) + (1-self.t).mm(self.begin), requires_grad=requires_grad) # (num_nodes-2)xD
