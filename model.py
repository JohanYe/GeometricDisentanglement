...
class VAE_dsprites2(nn.Module, EmbeddedManifold):
    def __init__(self, layers, latents_bases=None, latents_sizes=None, num_components=100, device=None):
        super(VAE_dsprites2, self).__init__()
    ...
    def fit_mean(self, data_loader, num_epochs=150, num_cycles=30, max_kl=5, batch_size=256, sampling_per_epoch=24):
        switch_epoch = num_epochs // 5
        optimizer = torch.optim.Adam(self.parameters(), lr=1e-4)
...
class VAE_cars3d(nn.Module, EmbeddedManifold):
    def __init__(self, layers, latents_bases=None, latents_sizes=None, num_components=100, device=None):
        super(VAE_cars3d, self).__init__()
        ...
        for cycle in range(1, num_cycles + 1):
            ...
            for epoch in range(num_epochs):
                ...
                if cycle < num_cycles - num_cycles // 5:
                    ...
                    lam = 1
                for batch_idx, data in enumerate(data_loader):
                    data = data.reshape(data.shape[0], int(data.shape[1] * data.shape[2])).to(self.device)
                    optimizer.zero_grad()
                    loss = -self.elbo(data, lam)
                    loss.backward()
                    optimizer.step()
